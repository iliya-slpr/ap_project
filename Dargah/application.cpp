...
BankAccount Application::getAcc(int cardNumber)
{
    for(int k=0;k<userList.size();k++)
    {
        for(int i=0;i<userList[k].getAccount().size();i++)
        {
            if(userList[k].getAccountPointer(i)->getCard().getCardNumber().toInt()==cardNumber)
            {
                return *userList[k].getAccountPointer(i);
            }
        }
...
BankAccount *Application::getAccount(int cardNumber)
{
    for(int k=0;k<userList.size();k++)
    {
        for(int i=0;i<userList[k].getAccount().size();i++)
...
QString Application::findNameByCard(int cardNumber)
{
    for(int k=0;k<userList.size();k++)
    {
        for(int i=0;i<userList[k].getAccount().size();i++)
...
Card *Application::getCardPointer(int cardNumber)
{
    for(int k=0;k<userList.size();k++)
    {
        for(int i=0;i<userList[k].getAccount().size();i++)
        {
            if(userList[k].getAccountPointer(i)->getCard().getCardNumber().toInt()==cardNumber)
            {
                return userList[k].getAccountPointer(i)->getCardPointer();
            }
        }
    }
    return new Card("0",0,QDate::currentDate(),SecurePassword(),0);
}
Card Application::getCard(int cardNumber)
{
    for(int k=0;k<userList.size();k++)
    {
        for(int i=0;i<userList[k].getAccount().size();i++)
        {
            if(userList[k].getAccount()[i].getCard().getCardNumber().toInt()==cardNumber)
            {
                return userList[k].getAccount()[i].getCard();
            }
        }
    }
    return Card("0",0,QDate::currentDate(),SecurePassword(),0);
}
